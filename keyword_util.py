from mogon_base_util import Products, Scraper_util, PLATFORM, RW_to_file, Search_obj

class Keyword(Search_obj):
    def __init__(self):
...
class Keyword_builder:
    ...
    def _set_and_build_dir(self):
        ...
        ### 設定目的地資料夾
        self.keyword_obj.prods_dir = f"keyword_search/{self.keyword_obj.keyword}/{self.keyword_obj.platform.value}"

        ### 建立products物件
        self.keyword_obj._init_search_obj(base_url=self.keyword_obj.keyword_url, prods_dir=self.keyword_obj.prods_dir)
...
def exp1_visit_3_platform_download_imgs_dummy(keyword):
    ...
    key_at_jraku.get_all_page_prods()                    ### 爬url存成prods.txt
    RW_to_file.write_prods_from_search_obj(key_at_jraku) ### 把 prods 存入檔案
    # RW_to_file.read_prods_to_search_obj(key_at_jraku)    ### 取得 之前存的 prods 檔案 ### 這行註解打開的話，要把前兩行註解掉喔！
    ...
    key_at_yahoobid.get_all_page_prods()                    ### 爬url存成prods.txt
    RW_to_file.write_prods_from_search_obj(key_at_yahoobid) ### 把 prods 存入檔案
    # RW_to_file.read_prods_to_search_obj(key_at_yahoobid)    ### 取得 之前存的 prods 檔案 ### 這行註解打開的話，要把前兩行註解掉喔！
    ...
    key_at_yahooshop.get_all_page_prods()                    ### 爬url存成prods.txt
    RW_to_file.write_prods_from_search_obj(key_at_yahooshop) ### 把 prods 存入檔案
    # RW_to_file.read_prods_to_search_obj(key_at_yahooshop)    ### 取得 之前存的 prods 檔案 ### 這行註解打開的話，要把前兩行註解掉喔！
...
def exp3_write_to_docx_dummy(platform, keyword):
    ...
    # RW_to_file.read_prods_to_search_obj(key_at_yahooshop)    ### 取得 之前存的 prods 檔案 ### 這行註解打開的話，要把前兩行註解掉喔！
    key_at_yahooshop.get_all_page_prods_img(prod_num=False, prod_title=True, shop_name=True, price=True)
    RW_to_file.write_prods_from_search_obj_to_word(key_at_yahooshop)
...
def exp5_manually_search_RIZING_mallet():
    ...
    searchs = [m_1000, m_3000, m_6011, m_6021, m_7001, m_611, m_2001, m_4000, m_5000, xg_series, 
               b_series, m_101, m_201, m_301, m_401, m_501, m_801, m_901, two_tone, m_01, sck, sc]
    
    combine_prods = Keyword_builder().build(PLATFORM.yahooshop, "combine")
    for search in searchs:
        search.get_all_page_prods()
        search.sort_by_key(sort_key="prod_title")
        combine_prods.products.prods +=  search.products.prods
    combine_prods.use_c_write_to_word(restart_img=True)
...
#     keyword_obj.get_all_page_prods() 
#     RW_to_file.write_prods_from_search_obj(keyword_obj) ### 把 shop.prods 存入檔案

# def step1b_read_from_file_and_download_imgs(keyword_obj):
#     RW_to_file.read_prods_to_search_obj(keyword_obj) ### 取得 之前存的 shop.prods 檔案
#     keyword_obj.get_all_page_prods_img()

# def step1c_read_from_file_and_write_to_word(keyword_obj):
#     RW_to_file.read_prods_to_search_obj(keyword_obj) ### 取得 之前存的 shop.prods 檔案
#     RW_to_file.write_prods_from_search_obj_to_word(keyword_obj)
